...
class FlowResult:
    ...
    def _run_inner(self) -> Any:
        ...
        run_store = LocalRunStore(database, db_path) if database is not None else None
        run_id = (
            run_store.new_run_id(self._flow_name) if run_store is not None
            else __import__("datetime").datetime.now().strftime("%Y%m%d_%H%M%S")
        )
        run_dir = (
            run_store.run_dir(self._flow_name, run_id)
            if run_store is not None
            else db_path / "flows" / self._flow_name / "runs" / run_id
        )
        ...
        if resume:
            ...
            latest = run_store.latest_run(self._flow_name) if run_store else None
            if latest is None:
                raise ValueError(
                    f"No previous runs found for flow '{self._flow_name}'"
                )
            prev_hashes = set(latest["task_hashes"])
        ...
        try:
            ...
            from rinnsal.orchestration.base import RunPlan

            outcome = LocalOrchestrator().run(
                RunPlan(
            ...
            n_passed = outcome.n_passed
            n_cached = outcome.n_cached
            n_failed = outcome.n_failed
            completed = outcome.completed
            failed_hashes = outcome.failed_hashes
    ...
    def _get_or_create_engine(self) -> Any:
        ...
        from rinnsal.compute.engine import (
            ExecutionEngine,
            set_engine,
            _default_engine,
        )
        ...
        from rinnsal.versioning.snapshot import SnapshotManager, set_snapshot_manager
                ...
                for i, (param, value) in enumerate(zip(task_params, task.args)):
                    if param in param_names:
                        if task.is_evaluated:
                            from rinnsal.modeling.expression import unwrap_value

                            arg_dict[param] = unwrap_value(value)
                        else:
                            from rinnsal.modeling.expression import ValueExpression
