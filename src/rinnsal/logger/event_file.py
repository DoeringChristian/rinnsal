"""Event file reader and writer for protobuf-based logging.

File format:
    Each record is stored as:
    - 4 bytes: message length (little endian uint32)
    - N bytes: serialized protobuf Event message

This format allows efficient append-only writes and streaming reads.
"""

from __future__ import annotations

import struct
import threading
from pathlib import Path
from typing import TYPE_CHECKING, Iterator

if TYPE_CHECKING:
    from rinnsal.logger.events_pb2 import Event


class EventFileWriter:
    """Writes Event messages to a file in length-prefixed format.

    Thread-safe for concurrent writes from multiple threads.
    """

    def __init__(self, path: Path | str):
        self._path = Path(path)
        self._path.parent.mkdir(parents=True, exist_ok=True)
        self._file = open(self._path, "ab")
        self._lock = threading.Lock()

    def write(self, event: Event) -> None:
        """Write an event to the file."""
        data = event.SerializeToString()
        length = struct.pack("<I", len(data))
        with self._lock:
            self._file.write(length)
            self._file.write(data)

    def flush(self) -> None:
        """Flush buffered data to disk."""
        with self._lock:
            self._file.flush()

    def close(self) -> None:
        """Close the file."""
        with self._lock:
            self._file.close()

    def __enter__(self) -> EventFileWriter:
        return self

    def __exit__(self, exc_type, exc_val, exc_tb) -> None:
        self.close()


class EventFileReader:
    """Reads Event messages from a file in length-prefixed format.

    Supports reading while the file is being written to.
    """

    def __init__(self, path: Path | str):
        self._path = Path(path)
        if not self._path.exists():
            raise FileNotFoundError(f"Event file not found: {path}")

    def __iter__(self) -> Iterator[Event]:
        """Iterate over all events in the file."""
        from rinnsal.logger.events_pb2 import Event

        with open(self._path, "rb") as f:
            while True:
                length_bytes = f.read(4)
                if len(length_bytes) < 4:
                    break

                length = struct.unpack("<I", length_bytes)[0]

                data = f.read(length)
                if len(data) < length:
                    break

                event = Event()
                event.ParseFromString(data)
                yield event

    def read_all(self) -> list[Event]:
        """Read all events from the file."""
        return list(self)

    def build_index(self) -> list[tuple[int, int, int, str, str]]:
        """Build an index of all events without fully parsing large blobs.

        Returns list of (offset, length, iteration, data_type, tag).
        The offset and length can be used to seek and read specific
        events on demand.
        """
        from rinnsal.logger.events_pb2 import Event

        index = []
        with open(self._path, "rb") as f:
            while True:
                offset = f.tell()
                length_bytes = f.read(4)
                if len(length_bytes) < 4:
                    break

                length = struct.unpack("<I", length_bytes)[0]
                data = f.read(length)
                if len(data) < length:
                    break

                event = Event()
                event.ParseFromString(data)
                data_type = event.WhichOneof("data")
                tag = ""
                if data_type == "scalar":
                    tag = event.scalar.tag
                elif data_type == "text":
                    tag = event.text.tag
                elif data_type == "figure":
                    tag = event.figure.tag
                elif data_type == "checkpoint":
                    tag = event.checkpoint.tag

                index.append(
                    (offset, length, event.iteration, data_type, tag)
                )

        return index

    def read_event_at(self, offset: int) -> Event:
        """Read a single event at the given file offset."""
        from rinnsal.logger.events_pb2 import Event

        with open(self._path, "rb") as f:
            f.seek(offset)
            length_bytes = f.read(4)
            length = struct.unpack("<I", length_bytes)[0]
            data = f.read(length)
            event = Event()
            event.ParseFromString(data)
            return event
