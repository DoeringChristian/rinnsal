...
class ContentHasher:
    """A stateful content hasher that can cache function hashes."""

    def __init__(self) -> None:
        self._func_cache: dict[int, str] = {}

    def hash_function(self, func: Callable[..., Any]) -> str:
        """Compute or retrieve a cached function hash."""
        func_id = id(func)
        if func_id not in self._func_cache:
            self._func_cache[func_id] = hash_function(func)
...
def get_hasher() -> ContentHasher:
    """Get the global content hasher instance."""
    return _hasher
